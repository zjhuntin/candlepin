        ...
        try {
            ...
            try {
                ...
                if (pemObj instanceof PEMKeyPair) {
                    return new JcaPEMKeyConverter().setProvider(BCFIPS).getKeyPair((PEMKeyPair) pemObj)
                        .getPrivate();
                }

                if (pemObj instanceof PEMEncryptedKeyPair) {
                    PEMEncryptedKeyPair encryptedInfo = (PEMEncryptedKeyPair) pemObj;
                    PEMDecryptorProvider provider = new JcePEMDecryptorProviderBuilder().setProvider(BCFIPS)
                        .build(getPassword());
                    PEMKeyPair decryptedInfo =  encryptedInfo.decryptKeyPair(provider);
                    return new JcaPEMKeyConverter().setProvider(BCFIPS).getPrivateKey(decryptedInfo
                        .getPrivateKeyInfo());
                }
