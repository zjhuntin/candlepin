...
public class X509CRLStreamWriterTest {
    ...
    private X509CRL readCRL(PublicKey signatureKey) throws Exception {
        ...
        changedCrl.verify(signatureKey, BouncyCastleFipsProvider.PROVIDER_NAME);
    ...
    public void testAddEntryToActualCRL() throws Exception {
        ...
        try {
            ...
            if (pemObj instanceof PEMKeyPair) {
                keyPair = new JcaPEMKeyConverter().setProvider(BCFIPS).getKeyPair((PEMKeyPair) pemObj);
            }
            else {
                crl.close();
    ...
    public void testAddEntryToEmptyCRL() throws Exception {
        ...
        X509CRL originalCrl = new JcaX509CRLConverter().setProvider(BCFIPS).getCRL(holder);
